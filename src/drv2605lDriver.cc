...
void Driver::initHaptics() {
  ...
  // Run LRA autocalibration sequence
  for (std::pair<uint8_t, uint8_t>& regVal : drv2605lLRACalRegs) {
    writeSuccess = writeSuccess ? writeReg(regVal.first, regVal.second) : false;
  }
...
uint8_t Driver::readReg(uint8_t addr) {
  int32_t res = i2c_smbus_read_byte_data(i2cBusFD, addr);
  if (res < 0) {
    std::cerr << "Failed to read register" << std::hex << addr;
    return 0;
  } else {
